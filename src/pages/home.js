...
export function homePage() {
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-6" data-stagger>
          <div class="why-card relative overflow-hidden rounded-2xl group hover:border-brand-gold/30 transition-all border border-white/10 min-h-[280px]">
            ...
            <div class="relative z-10 p-6 text-center h-full flex flex-col items-center justify-start gap-4">
              ...
              <h4 class="font-heading leading-tight text-balance text-xl font-semibold uppercase text-white min-h-[3.5rem] flex items-center">${t('home.why.premiumQuality.title')}</h4>
              <p class="text-brand-white text-base text-balance">${t('home.why.premiumQuality.desc')}</p>
            </div>
          </div>
          ...
          <div class="why-card relative overflow-hidden rounded-2xl group hover:border-brand-gold/30 transition-all border border-white/10 min-h-[280px]">
            ...
            <div class="relative z-10 p-6 text-center h-full flex flex-col items-center justify-start gap-4">
              ...
              <h4 class="font-heading leading-tight text-balance text-xl font-semibold uppercase text-white min-h-[3.5rem] flex items-center">${t('home.why.globalReach.title')}</h4>
              <p class="text-brand-white text-base text-balance">${t('home.why.globalReach.desc')}</p>
            </div>
          </div>
          ...
          <div class="why-card relative overflow-hidden rounded-2xl group hover:border-brand-gold/30 transition-all border border-white/10 min-h-[280px]">
            ...
            <div class="relative z-10 p-6 text-center h-full flex flex-col items-center justify-start gap-4">
              ...
              <h4 class="font-heading leading-tight text-balance text-xl font-semibold uppercase text-white min-h-[3.5rem] flex items-center">${t('home.why.wideRange.title')}</h4>
              <p class="text-brand-white text-base text-balance">${t('home.why.wideRange.desc')}</p>
            </div>
          </div>
          ...
          <div class="why-card relative overflow-hidden rounded-2xl group hover:border-brand-gold/30 transition-all border border-white/10 min-h-[280px]">
            ...
            <div class="relative z-10 p-6 text-center h-full flex flex-col items-center justify-start gap-4">
              ...
              <h4 class="font-heading leading-tight text-balance text-xl font-semibold uppercase text-white min-h-[3.5rem] flex items-center">${t('home.why.expertSupport.title')}</h4>
              <p class="text-brand-white text-base text-balance">${t('home.why.expertSupport.desc')}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="grid lg:grid-cols-2 gap-6" data-stagger>
          <a href="/news/featured" class="news-featured-card group card-hover block cursor-pointer overflow-hidden rounded-2xl">
            <div class="relative h-full min-h-[280px] sm:min-h-[420px]">
              <img src="/news/featured/featured-article-banner.png" alt="${t('news.featured.title')}"
                   class="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
              <div class="absolute inset-x-0 bottom-0 flex flex-col justify-end p-6 sm:p-8 bg-brand-gold">
                <span class="bg-black text-brand-gold text-xs font-medium uppercase tracking-wider px-3 py-1 rounded-full self-start mb-2">${t('news.featured.badge')}</span>
                <h3 class="font-heading text-xl sm:text-2xl font-semibold uppercase text-black mb-2
                           ">${t('news.featured.title')}</h3>
                <p class="text-black/70 text-base leading-relaxed mb-4 line-clamp-2">${t('news.featured.excerpt')}</p>
                <div class="flex items-center justify-between">
                  <span class="text-black/60 text-sm sm:text-base">${t('news.featured.date')}</span>
                  <span class="text-black text-sm sm:text-base font-medium flex items-center gap-2
          ...
          <div class="flex flex-col gap-6" id="home-news-articles">
  ...
  return {
    ...
    init() {
      ...
      // Overlay CMS settings if available
      getSettings().then(settings => {
        if (!settings) return;

        // Update stats from CMS
        if (settings.company?.stats) {
          ...
          statEls.forEach((el, i) => {
            if (statValues[i] !== undefined) {
              el.setAttribute('data-count', statValues[i]);
            }
          });
        }
      });

      // Load latest news from API for the news section
