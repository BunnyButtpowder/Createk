...
def read_detail_sheet() -> pd.DataFrame:
    ...
    # The header is on row 0 (default)
...
def fill_missing_values(df: pd.DataFrame, columns: List[str]) -> pd.DataFrame:
    """Fill missing values in specified columns by carrying forward previous non-null values."""
    df_filled = df.copy()
    for col in columns:
...
def main():
    ...
    # Fill missing category and group information in detail sheet
    print(f"\nFilling missing category/group information...")
    df_details = fill_missing_values(df_details, ['Bộ phận', 'Nhóm'])
